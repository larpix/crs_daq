...
v2a_nonrouted_channels = [6, 7, 8, 9, 22,
...
def parse_toggle_json(toggle_json):
    ...
    toggle_list = {}
    with open(toggle_json, 'r') as f:
        toggle_list = json.load(f)
    ...
    for file in files:
        ...
        if chip_key in toggle_list.keys():
            ...
            if 'meta' in config.keys():
                ...
                if not 'toggle_lists' in config['meta'].keys():
                    config['meta']['toggle_lists'] = []
...
if __name__ == "__main__":
    ...
    parser.add_argument('--toggle_json', type=str, default=None,
                        help='''List of (channel, toggle_value) to merge to config pixel trims''')
    parser.add_argument('--toggle_global', action='store_true',
                        default=False, help='''Allow freedom to change global DACs also''')
    parser.add_argument('--cryo', action='store_true', default=False,
                        help='''If toggle_global also set, adjust trim_scale for cryo''')
