...
_initial_global_dac = 50  # 100 ### initial global DAC set value
...
_maxdac_global = 50
_mindac_global = 5  # re-evaluate chip if global DAC set below this value
    ...
    for chip_key in c.chips:
        if c[chip_key].asic_version == 2:
            ...
            if asic_id in csa_disable.keys():
                for chan in _v2a_nonrouted:
                    csa_disable[asic_id].append(chan)
            else:
...
def initial_asic_config(c, chips):
    ...
    for chip in chips:
        ...
        c[chip].config.dropped_packets = 15  # register 120 ADC hold delay
        c[chip].config.periodic_reset_cycles = 4  # registers [163-165]
        chip_register_pairs.append((chip, [64, 82, 120, 128, 163, 164, 165]))

    # ok, diff = c.enforce_configuration(
    #        chips, timeout=0.1, connection_delay=0.05, n=2, n_verify=1)
    # if not ok:
...
def enable_frontend(c, chips_to_test, csa_disable, all_network_keys, pacman_configs):
    ...
    while True:
        ...
        for chip_key in current_chips:
            if chip_key not in chips_to_test:
                continue
        ...
        # ok, diff = c.enforce_configuration(
        #        current_chips, timeout=0.1, connection_delay=0.05, n=2, n_verify=1)
...
def mask_chip(c, chip, chip_register_pairs):
    ...
    chip_register_pairs.append(
        (chip, list(range(66, 74))+list(range(131, 139))))
...
def silence_highest_rate(c, chip, channel_rate, toggle_global_csa_disable):
    ...
    if highest_channel != None:
        ...
        if asic_id not in toggle_global_csa_disable.keys():
            toggle_global_csa_disable[asic_id] = []
...
def toggle_global_dac(c, toggled_chips, csa_disable, all_network_keys, pacman_configs, max_channel_fraction=0.8):
    ...
    while high_rate:
        ...
        print('\n\niteration {}, packet count {}'.format(
            iteration, all_packets.shape[0]))
        if all_packets.shape[0] == 1025:
        ...
        if all_packets.shape[0] == 0:
            ...
            packets = all_packets[np.vectorize(
                lambda x: x.packet_type)(all_packets) == 1]

        if packets.shape[0] > 0:
            iog_triggers = np.vectorize(
                lambda x: x.io_group)(packets).astype(int)
            ioc_triggers = np.vectorize(
                lambda x: x.io_channel)(packets).astype(int)
            chip_triggers = np.vectorize(
                lambda x: x.chip_id)(packets).astype(int)
            channel_triggers = np.vectorize(
                lambda x: x.channel_id)(packets).astype(int)
            ...
            if all_lowered_active_chips > 0:
                ...
                print('Lowering threshold on all active chips\t',
                      all_lowered_active_chips)
                print('masked count: ', len(masked_chip_set), '\n')
            high_rate = True
            if all_lowered_active_chips == 0:
                high_rate = False
                print('No remaining active chips. Exiting at iteration ', iteration)
            continue
        ...
        for chip in toggled_chips:
            ...
            if chip in triggered_chips:
                ...
                mask = np.logical_and(
                    iog_triggers == ids[0], ioc_triggers == ids[1])
                ...
                if asic_id in toggle_global_csa_disable.keys():
                    channel_intersection = list(set_channels & set(
                        toggle_global_csa_disable[asic_id]))
                    if len(channel_intersection) > 0:
                        if chip not in offenders.keys():
                            offenders[chip] = {}
                        for cid in channel_intersection:
                            if cid not in offenders[chip].keys():
                                offenders[chip][cid] = 1
                            else:
                                offenders[chip][cid] += 1
                ...
                for channel in set_channels:
                    channel_rate[channel] = np.sum(
                        channel == channels)/_runtime_global
                ...
                for channel in kill_channel:
                    ...
                    chip_register_pairs.append(
                        (chip, list(range(66, 74))+list(range(131, 139))))
                if len(kill_channel) != 0:
                    continue  # re-assess chip if channel disabled; DO NOT toggle global DAC
                ...
                if chip_rate > _minrate_global and chip_rate < _maxrate_global:
                    if c[chip].config.threshold_global > _maxdac_global:
                        silence_highest_rate(
                            c, chip, channel_rate, toggle_global_csa_disable)
                        chip_register_pairs.append(
                            (chip, list(range(66, 74))+list(range(131, 139))))
                        continue
                    else:
                ...
                if chip_rate < _minrate_global:
                    if packets.shape[0] > _maxtriggers:
                        high_rate = True
                        continue
                    decrement_count += 1
                    if c[chip].config.threshold_global == 0:
        ...
        for ck in offenders.keys():
            for cid in offenders[ck]:
                if offenders[ck][cid] > _bail_threshold:
                    print('BAIL ========= ', ck, ' ', cid,
                          ' fired ', offenders[ck][cid])
                    high_rate = False
                    break
            if high_rate == False:
                break

        print('==>\ttriggered: ', len(triggered_chips), '\tuntriggered: ', untriggered_count, '\t resolved: ', len(masked_chip_set), '\t total: ', len(
        ...
        for key in toggle_global_csa_disable.keys():
            haha += len(set(toggle_global_csa_disable[key]))
        print('==>\tCSA disable size: ', haha, '\t keys: ',
              len(toggle_global_csa_disable.keys()), '\n')

    for chip in toggled_chips:
        if chip not in masked_chip_set:
            chip_reevaluate_queue.add(chip)
...
def main(pacman_config,
    ...
    for io_group_ip_pair in pacman_configs['io_group']:
        ...
        all_network_keys += enforce_parallel.get_chips_by_io_group_io_channel(
            utility_base.get_from_json(network_config_paths_file_, io_group))
    ...
    save_to_json(diagnostic_dir+'/input_disabled_list_{}'.format(
        time.strftime("%Y_%m_%d_%H_%M_%S_%Z")), csa_disable, recast=False)
    ...
    if chip_list == 'None':
        ...
        chips_to_test = chips_to_test_from_file(chip_list)
    ...
    timeStart = time.time()
    enable_frontend(c, chips_to_test, global_disable,
                    all_network_keys, pacman_configs)
    timeEnd = time.time()-timeStart
    print('\t==> %.2f seconds --- enable channel front-ends\n' % timeEnd)
    time_log['enable_frontend'] = timeEnd

    timeStart = time.time()
    global_track_triggers, toggle_global_csa_disable, global_track_progress, chip_reevaluate = toggle_global_dac(
        c, chips_to_test, global_disable, all_network_keys, pacman_configs)
    timeEnd = time.time()-timeStart
    print('\t==> %.2f seconds --- toggle global DACs\n' % timeEnd)
    save_to_json(diagnostic_dir+'/global_disabled_list_{}'.format(
        time.strftime("%Y_%m_%d_%H_%M_%S_%Z")), toggle_global_csa_disable, recast=False)
    time_log['toggle_global'] = timeEnd

    after_global = {}
    for key in global_disable.keys():
        if key not in after_global:
            after_global[key] = []
        for chan in global_disable[key]:
            if chan not in after_global[key]:
                after_global[key].append(chan)
    for key in toggle_global_csa_disable.keys():
        if key not in after_global:
            after_global[key] = []
    ...
    save_to_json(diagnostic_dir+'/combined_global_disabled_list_{}'.format(
        time.strftime("%Y_%m_%d_%H_%M_%S_%Z")), global_disable, recast=False)
    save_to_json(diagnostic_dir+'/global_configuration_{}'.format(
        time.strftime("%Y_%m_%d_%H_%M_%S_%Z")), global_track_progress, recast=True)
    save_to_json(diagnostic_dir+'/global_triggers_{}'.format(
        time.strftime("%Y_%m_%d_%H_%M_%S_%Z")), global_track_triggers, recast=True)
    ...
    if this_chip_count > 0:
        save_chip_key_list(diagnostic_dir+'/chip_reevaluation_list_{}'.format(
            time.strftime("%Y_%m_%d_%H_%M_%S_%Z")), chips_to_test)
