...
_maxtriggers=200000 #1e7 #200000. ### do not lower global DACs if total packet count exceeds this value
_bail_threshold=3
...
def toggle_global_dac(c, toggled_chips, csa_disable, all_network_keys, pacman_configs, max_channel_fraction=0.8):
    ...
    while high_rate:
        ...
        if all_packets.shape[0]>_maxtriggers: break
        ...
        for chip in toggled_chips:
            ...
            if chip in triggered_chips:
                ...
                for channel in kill_channel:
                    ...
                    if c[chip].config.pixel_trim_dac[channel]<16:
                        c[chip].config.pixel_trim_dac[channel]=16
                        chip_register_pairs.append( (chip, list(range(64))) )
                        continue
                    if c[chip].config.pixel_trim_dac[channel]==16:
                        c[chip].config.pixel_trim_dac[channel]=24
                        chip_register_pairs.append( (chip, list(range(64))) )
                        continue
                    if c[chip].config.pixel_trim_dac[channel]==24:
                        c[chip].config.pixel_trim_dac[channel]=31
                        chip_register_pairs.append( (chip, list(range(64))) )
                        continue
                    if c[chip].config.pixel_trim_dac[channel]==31:                    
                    ...
                    if c[chip].config.threshold_global>_maxdac_global:
                        ...
                        for channel in channel_rate:
                            if channel_rate[channel]>highest_rate:
                                highest_rate=channel_rate[channel]
                                highest_channel=int(channel)
                        if c[chip].config.pixel_trim_dac[highest_channel]<16:
                            c[chip].config.pixel_trim_dac[highest_channel]=16
                            chip_register_pairs.append( (chip, list(range(64))) )
                            continue
                        if c[chip].config.pixel_trim_dac[highest_channel]==16:
                            c[chip].config.pixel_trim_dac[highest_channel]=24
                            chip_register_pairs.append( (chip, list(range(64))) )
                            continue
                        if c[chip].config.pixel_trim_dac[highest_channel]==24:
                            c[chip].config.pixel_trim_dac[highest_channel]=31
                            chip_register_pairs.append( (chip, list(range(64))) )
                            continue
                        if c[chip].config.pixel_trim_dac[highest_channel]==31:                    
                            channel_mask_count+=1
                            c[chip].config.channel_mask[int(channel)]=1
                            c[chip].config.csa_enable[int(channel)]=0
                            if asic_id not in toggle_global_csa_disable.keys(): toggle_global_csa_disable[asic_id]=[]
...
#                        chip_register_pairs.append( (chip, list(range(66,74))+list(range(131,139))) )
        ...
        print('==>\tCSA DISABLED  ---  unique channels: ',haha,'\t unique asics: ',len(toggle_global_csa_disable.keys()),'\n')
...
def main(pacman_config,
    ...
    #diagnostic_dir = f'iog_{io_group}-'+diagnostic_dir
    timeStart = time.time()
