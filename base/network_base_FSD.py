...
def append_upstream_chip_ids(io_channel, chip_id, waitlist):
    ...
    if io_channel in list(range(1,33,4)):
        for i in range(chip_id,51):
            addendum.add(i); addendum.add(i-10); addendum.add(i+10); addendum.add(i+20) 
    ...
    if io_channel in list(range(4,33,4)):
        for i in range(chip_id,171):
            addendum.add(i); addendum.add(i-20); addendum.add(i-10); addendum.add(i+10)
    return addendum
...
#                if last_chip_id==80 and daughter_id in [70,90,81]: bail=True
                ...
                if daughter_id<11 or daughter_id>170: continue
